...
const unsigned long WIFI_TIMEOUT_MS = 30000; // 30초 동안 연결 안되면 재시도
...
void connectWiFi() {
  ...
  // --- Static IP 설정 (DHCP IP 할당 절차를 스킵하여 연결 속도 1~2초 가속화) ---
  // ⚠️ 야외 핫스팟 등 환경이 바뀔 때는 이 아래 config 설정을 주석-해제하여 사용하세요.
  /*
  IPAddress local_IP(192, 168, 0, 210);   // 임의 고정 IP
  IPAddress gateway(192, 168, 0, 1);     // 기본 게이트웨이
  IPAddress subnet(255, 255, 255, 0);   // 서브넷 마스크
  IPAddress primaryDNS(8, 8, 8, 8);     // DNS 1 (Vercel 도메인 확인용)
  IPAddress secondaryDNS(1, 1, 1, 1);   // DNS 2

  if (!WiFi.config(local_IP, gateway, subnet, primaryDNS, secondaryDNS)) {
    Serial.println("[WiFi] Static IP 설정 실패");
  }
  */

  WiFi.persistent(false); // 플래시 저장 생략 → 부팅 딜레이 제거
  WiFi.mode(WIFI_STA);
  WiFi.setSleep(false);   // 절전모드 끄기 → 연결 안정성 개선
...
void sendTaskComplete(int task_id) {
  ...
  client.setInsecure(); // Vercel HTTPS 보안 연결 허용
  HTTPClient http;

  String url = "https://" + String(VERCEL_HOST) + "/api/complete-task?task_id=" + String(task_id);

  if (http.begin(client, url)) {
    int httpResponseCode = http.POST(""); // 빈 데이터 POST 요청
    if (httpResponseCode == 200) {
      Serial.print("[Task] 서버 완료 전송 성공. ID: ");
      Serial.println(task_id);
    } else {
      Serial.print("[Task] 서버 완료 전송 실패. 코드: ");
...
// ============================================================
//  [함수] executeGCode(gcode, task_id)
//  - 실행 중 LED 초록 유지, 완료 후 LED 파랑
// ============================================================
void executeGCode(const String& gcode, int task_id) {
